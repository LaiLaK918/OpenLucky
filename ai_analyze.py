...
class AttachAlgoOrds(BaseModel):
    ...
    tpOrdPx: str = Field(
        "-1", description="Take profit order price, -1 for market order"
    )
    ...
    tpTriggerPxType: TriggerPriceType = Field(
        TriggerPriceType.MARK, description="Take profit trigger price type"
    )
    slTriggerPxType: TriggerPriceType = Field(
        TriggerPriceType.MARK, description="Stop loss trigger price type"
    )
...
class TradeOrder(BaseModel):
    ...
    instId: Optional[str] = Field(
        None, description="Trading pair symbol, e.g., BTC-USDT-SWAP"
    )
    tdMode: TradingMode = Field(
        TradingMode.CROSS, description="Margin mode, fixed as cross"
    )
    side: Optional[TradeSide] = Field(None, description="Trading side: buy or sell")
    ordType: OrderType = Field(
        OrderType.MARKET, description="Order type, fixed as market"
    )
    sz: Optional[str] = Field(None, description="Contract size")
    lever: Optional[str] = Field(None, description="Leverage multiplier")
    attachAlgoOrds: Optional[AttachAlgoOrds] = Field(
        None, description="Take profit and stop loss orders"
    )
...
class ClosePosition(BaseModel):
    ...
    instId: Optional[str] = Field(
        None, description="Trading pair symbol to close position"
    )
    posSide: PositionSide = Field(
        PositionSide.NET, description="Position side, fixed as net"
    )
...
class TradingDecision(BaseModel):
    ...
    trades: Optional[List[TradeOrder]] = Field(
        None, description="List of trading orders to execute"
    )
    closures: Optional[List[ClosePosition]] = Field(
        None, description="List of positions to close"
    )
...
class AIAnalyzer:
    ...
    def __init__(self, config_path: str = "config.ini"):
        ...
        self.model = self.config.get(
            "XAI", "model", fallback="grok-4-fast-non-reasoning"
        )
        self.client = Client(
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise Exception(
                        f"Failed to validate response structure: {validation_error}"
                    )
    ...
    def run_analysis(self, output_file: str = "decision.json") -> dict:
        ...
        if decision.get("trades"):
            ...
            for i, trade in enumerate(decision["trades"], 1):
                print(
                    f"  {i}. {trade.get('side', 'N/A')} {trade.get('instId', 'N/A')} "
